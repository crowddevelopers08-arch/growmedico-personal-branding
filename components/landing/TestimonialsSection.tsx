    ...
    <div className="flex flex-col overflow-hidden rounded-3xl border border-[rgba(51,189,176,0.14)] bg-[#141c1a]">
      ...
      <div className="flex flex-1 flex-col gap-3 px-4 py-4">
        ...
        <div className="flex items-baseline gap-2">
          ...
          <div className="text-[0.6rem] uppercase tracking-[0.06em] text-[#4a6460]">{item.label}</div>
        </div>

        {/* Quote */}
        <div className="text-[0.75rem] font-light leading-[1.6] text-[#d7ebe7]">&quot;{item.quote}&quot;</div>
      </div>
    </div>
  );
}
...
export function TestimonialsSection() {
  ...
  const visibleIndices = Array.from({ length: visibleCount }, (_, i) => (active + i) % total);
  const cardsRowClass =
    total >= 3
      ? "grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6"
      : "mx-auto grid max-w-[860px] grid-cols-1 gap-5 sm:grid-cols-2 lg:gap-6";
